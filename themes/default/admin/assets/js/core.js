...
$(document).ready(function () {
    ...
    $('#random_num').click(function () {
        let code_textBox = $(this).parent('.input-group').children('input');
        let product_code = '';
        if (getCategoryCode()) {
            product_code = getCategoryCode() + generateCardNo(8);
        } else {
            product_code = generateCardNo(8);
        }
        code_textBox.val(product_code);
    });
...
$(document).ready(function () {
    $(document).on('click', '.row_status', function (e) {
        ...
        if (row.hasClass('invoice_link')) {
            ...
            $('#myModal').modal('show');
        } else if (row.hasClass('webquote_link')) {
            $('#myModal').modal({
                remote: site.base_url + 'webquotes/update_status/' + id
            });
            $('#myModal').modal('show');
...
$(document).ready(function () {
    ...
    // Start Web Quotation Link Modified By Anam
    $('body').on('click', '.webquote_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').modal({
            remote: site.base_url + 'webquotes/modal_view/' + $(this).parent('.webquote_link').attr('id')
        });
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'webquotes/view/' + $(this).parent('.webquote_link').attr('id');
    });
    $('body').on('click', '.webquote_link2', function () {
        $('#myModal').modal({
            remote: site.base_url + 'webquotes/modal_view/' + $(this).attr('id')
        });
        $('#myModal').modal('show');
    });
    // END of Web Quotation Link Modified By Anam
