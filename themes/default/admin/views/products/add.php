...
<script type="text/javascript">
    function getCategoryCode() {
        let cat_name = $('#category').children(':selected').text();
        let cat_code = cat_name.replace(/[^a-zA-Z]+/g, '').substr(0, 4).toUpperCase();
        return cat_code;
    }
    function setProductCode () {
        let code_textBox = $('#code');
        let digits = code_textBox.val().match(/\d+/);
        let product_code = '';
        if (digits){
            product_code = getCategoryCode() + digits;
        } else {
