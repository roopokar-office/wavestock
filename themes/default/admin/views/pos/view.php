                ...
                <table class="table table-condensed">
                    <tbody>
                        ...
                        foreach ($rows as $row) {
                            ...
                            echo '<tr><td colspan="2" class="no-border">#' . $r . ': &nbsp;&nbsp;' . $CI->get_brand_name($row->product_id).' '.$row->product_name;
                            
                            if($row->comment!=NULL) echo " (".$row->comment.")";
                            if($row->discount != 0) echo " (orginal price: ".$this->sma->formatQuantity($row->unit_price+$row->item_discount).")";
                            echo ($row->variant ? ' (' . $row->variant . ')' : '') . '<br>'; 
                            //if($row->product_type=='combo') echo var_dump($row);
