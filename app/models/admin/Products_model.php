...
class Products_model extends CI_Model
{
    ...
    public function getWarehouseExpiryList($w_id,$p_id)
    {
        $this->db->select('*');
        $this->db->from('sma_purchase_items as pi, sma_purchases as p')
        ->where(array('pi.warehouse_id' => $w_id,'product_id' => $p_id))
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $tags)
    {
        if ($this->db->insert('products', $data)) {
            ...
            if ($tags){
                $this->tags_model->addTagsByProduct($product_id, $tags);
            }
