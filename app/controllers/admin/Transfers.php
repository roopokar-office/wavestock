...
class Transfers extends MY_Controller
{
    ...
    $edate = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;
        ...
        if ($sdate OR $edate) { 
                $this->datatables
                    ->where('CONVERT(date, date) >=', $sdate)
                    ->where('CONVERT(date, date) <=', $edate);
        }
            
        if (!$this->Owner && !$this->Admin && !$this->session->userdata('view_right')) {
           // $this->datatables->where('created_by', $this->session->userdata('user_id'));
           $this->datatables
                    ->where('from_warehouse_id', $this->session->userdata('warehouse_id'))
                    ->or_where('to_warehouse_id', $this->session->userdata('warehouse_id'));
        }
