...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("sma_quotes.id AS qid, date, reference_no,  GROUP_CONCAT(sma_quote_items.product_name SEPARATOR ','), biller, customer, supplier, grand_total, status, attachment")
                ->from('quotes')
                ->where('sma_quotes.warehouse_id', $warehouse_id)
                ->join('sma_quote_items','sma_quotes.id=sma_quote_items.quote_id')
                ->group_by('sma_quote_items.quote_id');
        } else {
            $this->datatables
                ->select("sma_quotes.id AS qid, date, reference_no,  GROUP_CONCAT(sma_quote_items.product_name SEPARATOR ','), biller, customer, supplier, grand_total, status, attachment")
                ->from('sma_quotes')
    ...
    {
        $data = $this->quotes_model->getBrandNameByProduct($id);
