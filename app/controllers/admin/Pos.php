...
class Pos extends MY_Controller
{
    ...
    {
        $data = $this->pos_model->getProductComboItemsforPOS($id);
        $data = json_decode(json_encode($data),true);
        //var_dump($data);
        echo 'Combo Products:<br>';        
        foreach ($data as $key => $value) {
            echo $data[$key]['name'].' (Qty: '.$data[$key]['qty'].') <br>';
        }
    }
    
      public function get_brand_name($id = null)
    {
        $data = $this->pos_model->getBrandNameByProduct($id);
        $data = json_decode(json_encode($data),true);
                
        foreach ($data as $key => $value) {
            return $data[$key]['bname'];
        }
    }
