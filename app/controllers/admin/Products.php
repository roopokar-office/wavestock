...
class Products extends MY_Controller
{
    ...
    function product_list_filter($warehouse_id = NULL)
    {
        $this->sma->checkPermissions();
        ...
        $this->data['supplier'] = $this->input->get('supplier') ? $this->site->getCompanyByID($this->input->get('supplier')) : NULL;
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('products')));
        $meta = array('page_title' => lang('products List Filter'), 'bc' => $bc);
        $this->page_construct('products/product_list_filter', $meta, $this->data);
    }
...
//        $supplier = $this->input->get('supplier') ? $this->input->get('supplier') : NULL;
        ...
        /*if ($warehouse_id) {
         ...
         //if ($brand) {
            ...
            $this->datatables->where('products.subcategory_id =', $subcategory );
            }
            if ($warehouse){

            $this->datatables->where('products.warehouse =', $warehouse );
            } 
            if ($product){
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        $this->load->library('datatables');
        $pending_qty_table = "(
                SELECT product_id, SUM(quantity) AS pending_qty
                FROM {$this->db->dbprefix('purchase_items')} WHERE status != 'received'
                GROUP BY product_id
                                ) pq";
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, IFNULL(pq.pending_qty, 0) as pending_qty, wp.rack as rack, alert_quantity", FALSE)
            ->from('products');
            if ($this->Settings->display_all_products) {
                $this->datatables->join("( SELECT product_id, quantity, rack from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id', 'left');
            } else {
            ...
            ->join('units', 'products.unit=units.id', 'left')
            ->join('brands', 'products.brand=brands.id', 'left')
            ->join($pending_qty_table, 'products.id=pq.product_id', 'left');
            // ->group_by("products.id");
        } else {
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(quantity, 0) as quantity, pq.pending_qty as pending_qty, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity", FALSE)
                ->from('products')
                ->join('categories', 'products.category_id=categories.id', 'left')
                ->join('units', 'products.unit=units.id', 'left')
                ->join('brands', 'products.brand=brands.id', 'left')
        ...
        $this->datatables->add_column("Action", $action, "productid, image, code, name");
        echo $this->datatables->generate();
    }
    ...
    public function get_expiry_warehouse_items($wid = null,$pid = null)
    {
        $data = $this->products_model->getWarehouseExpiryList($wid,$pid);
        $data = json_decode(json_encode($data),true);
        
        foreach ($data as $key => $value) {
            $list.= 'LOT: '.$data[$key]['reference_no'].' expires <b>'.$data[$key]['expiry'].'</b><i> ('.$this->sma->formatQuantity($data[$key]['quantity']).' pcs)</i><br>';
        }
        return $list;
    }
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $tags)) {
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
            $this->tags_model->updateTagsByProduct($id, $this->input->post('tags'));
            ...
            $this->data['brands'] = $this->site->getAllBrands();
            $this->data['tags'] = $this->site->getAllTags();
            $this->data['selected_tags'] = $this->tags_model->getSelectedTags($id);
            $this->data['base_units'] = $this->site->getAllBaseUnits();
            $this->data['warehouses'] = $warehouses;
