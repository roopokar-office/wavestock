...
class Sales extends MY_Controller
{
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, sma_sales.reference_no, GROUP_CONCAT(sma_sale_items.product_name SEPARATOR '<br>'), biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sma_sales')
                ->where('sma_sales.warehouse_id', $warehouse_id)
                ->join('sma_sale_items','sma_sales.id=sma_sale_items.sale_id')
                ->group_by('sma_sale_items.sale_id');
        } else {
            $this->datatables
                ->select("{$this->db->dbprefix('sales')}.id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, sma_sales.reference_no, GROUP_CONCAT(sma_sale_items.product_name SEPARATOR '<br>'), biller, {$this->db->dbprefix('sales')}.customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sma_sales')
    ...
    public function get_combopoduct_name($id = null)
    {
        $data = $this->sales_model->getProductComboItems($id);
        $data = json_decode(json_encode($data),true);
        echo '<br>Combo Products:<br>';        
        foreach ($data as $key => $value) {
            echo $data[$key]['name'].' (Qty: '.substr($data[$key]['qty'], 0, -5).') <br>';
        }
    }
    
    public function get_brand_name($id = null)
    {
        $data = $this->sales_model->getBrandNameByProduct($id);
        $data = json_decode(json_encode($data),true);
