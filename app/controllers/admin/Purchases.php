...
class Purchases extends MY_Controller
{
    ...
    public function getPurchases($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("sma_purchases.id AS qid, DATE_FORMAT(sma_purchases.date, '%Y-%m-%d %T') as date, reference_no, GROUP_CONCAT(sma_purchase_items.product_name SEPARATOR ','), sma_purchases.supplier, sma_purchases.status, sma_purchases.grand_total, sma_purchases.paid, (sma_purchases.grand_total-sma_purchases.paid) as balance, sma_purchases.payment_status, sma_purchases.attachment")
                ->from('sma_purchases')
                ->where('sma_purchases.warehouse_id', $warehouse_id)
                ->join('sma_purchase_items','sma_purchases.id=sma_purchase_items.purchase_id')
                ->group_by('sma_purchase_items.purchase_id');
        } else {
            $this->datatables
                ->select("sma_purchases.id AS qid, DATE_FORMAT(sma_purchases.date, '%Y-%m-%d %T') as date, reference_no, GROUP_CONCAT(sma_purchase_items.product_name SEPARATOR '<br>'), sma_purchases.supplier, sma_purchases.status, sma_purchases.grand_total, sma_purchases.paid, (sma_purchases.grand_total-sma_purchases.paid) as balance, sma_purchases.payment_status, sma_purchases.attachment")
                ->from('sma_purchases')
                ->join('sma_purchase_items','sma_purchases.id=sma_purchase_items.purchase_id')
                ->group_by('sma_purchase_items.purchase_id');
        }
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue($char++.'1', lang('supplier'));
                    $this->excel->getActiveSheet()->SetCellValue($char++.'1', lang('status'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue($char++ . $row, $this->sma->hrld($purchase->date));
                        $this->excel->getActiveSheet()->SetCellValue($char++ . $row, $purchase->reference_no);
                        $this->excel->getActiveSheet()->SetCellValue($char++ . $row, $purchase->supplier);
                        $this->excel->getActiveSheet()->SetCellValue($char++ . $row, $purchase->status);
                        $this->excel->getActiveSheet()->SetCellValue($char++ . $row, $this->sma->formatMoney($purchase->grand_total));
                        $row++;
                    }
