...
class Reports extends MY_Controller
{
    ...
    function expiry_alerts_lot($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('expiry_alerts_lot');
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('reports'), 'page' => lang('reports')), array('link' => '#', 'page' => lang('lot_based_product_expiry_alerts')));
        $meta = array('page_title' => lang('product_expiry_alerts'), 'bc' => $bc);
        $this->page_construct('reports/expiry_alerts_lot', $meta, $this->data);
    }
    ...
    function getExpiryAlerts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ->select("image, product_code, product_name, quantity_balance, warehouses.name, expiry, expiry")
                ->from('purchase_items')
                ->join('products', 'products.id=purchase_items.product_id', 'left')
                ->join('warehouses', 'warehouses.id=purchase_items.warehouse_id', 'left')
                ->where('expiry !=', NULL)->where('expiry !=', '0000-00-00')
    ...
    function getExpiryAlertslot($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('expiry_alerts', TRUE);
        $date = date('Y-m-d', strtotime('+3 months'));
        ...
        if ($warehouse_id) {
            $this->datatables
                //->select("image, product_code, product_name, quantity_balance, warehouses.name, expiry")
                ->select("product_code, product_name, reference_no, warehouses.name, expiry")
                ->from('purchase_items')
                ->join('products', 'products.id=purchase_items.product_id', 'left')
                ->join('warehouses', 'warehouses.id=purchase_items.warehouse_id', 'left')
    ...
    function getProductsExpiryfilter()
    {
        $this->sma->checkPermissions('index', TRUE);
        $warehouse_id = $this->input->get('warehouse') ? $this->input->get('warehouse') : NULL;
        $sdate = $this->input->get('start_date') ? $this->input->get('start_date') : NULL;
        $edate = $this->input->get('end_date') ? $this->input->get('end_date') : NULL;

        $this->load->library('datatables');
        if ($warehouse_id OR $sdate OR $edate) {
        ...
        $this->datatables
                ...
                $this->datatables->where('expiry !=', NULL)->where('expiry !=', '0000-00-00')
                ->where('quantity_balance >', 0)
                 ->where('expiry >', $sdate)
                ->where('expiry <', $edate);
        } else {
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        $tag = $this->input->get('tag') ? $this->input->get('tag') : NULL;
        ...
        $tp = "( SELECT prod.id from " . $this->db->dbprefix('products') . " prod INNER JOIN " . $this->db->dbprefix('tags_products') . " tagprod on prod.id = tagprod.sale_id ";
        ...
        if ($pdf || $xls) {
            ...
            if ($tag) {
                $this->db
                    ->join($this->db->dbprefix('tags_products'), 'products.id = tags_products.product_id')
                    ->where('tags_products.tag_id', $tag);
            }
            $this->db
            ...
            if ($tag) {
                $this->datatables
                    ->join($this->db->dbprefix('tags_products'), 'products.id = tags_products.product_id', 'inner')
                    ->where('tags_products.tag_id', $tag);
            }
            $this->datatables
                ->join($sp, 'products.id = PSales.product_id', 'left')
